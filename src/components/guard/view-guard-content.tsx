...
import { Star, MapPin, Battery, Wifi, Clock, Shield, User, CheckCircle, XCircle, AlertCircle, Loader2 } from "lucide-react";
...
import {
    fetchGuardCurrentStatus,
    clearGuardStatus
} from "@/store/slices/shiftLogsSlice";

export default function ViewGuardContent({ guard }: ViewGuardTopCardProps) {
    const dispatch = useAppDispatch();
    const { currentGuardStatus, isLoading } = useAppSelector((state) => state.shiftLogs);
    const [isStatusLoading, setIsStatusLoading] = useState(false);

    useEffect(() => {
        if (guard?.id) {
            loadGuardStatus(guard.id);
        }
        return () => {
            dispatch(clearGuardStatus());
        };
    }, [guard?.id]);

    const loadGuardStatus = async (guardId: number) => {
        setIsStatusLoading(true);
        try {
            await dispatch(fetchGuardCurrentStatus(guardId));
        } catch (error) {
    ...
    const getAssignmentStatusDisplay = () => {
        if (isStatusLoading) {
            return {
                text: 'Checking...',
        ...
        if (currentGuardStatus.has_active_assignment) {
            return {
    ...
    const assignmentStatus = getAssignmentStatusDisplay();
        ...
        <div className="">
            <Tabs defaultValue="personal" className="w-full">
                ...
                <div className="overflow-hidden border rounded-2xl mb-3">
                    ...
                    <div className="px-4 sm:px-6 py-2 bg-blue-50 dark:bg-blue-900/20 border-b flex flex-wrap items-center justify-between gap-2">
                        <div className="flex items-center gap-4 flex-wrap">
                            <div className="flex items-center gap-2">
                            ...
                            <Badge 
                                variant="outline" 
                                className={`${assignmentStatus.color} border-none flex items-center gap-1`}
                            >
                                {assignmentStatus.icon}
                                <span className="text-xs font-medium">{assignmentStatus.text}</span>
                            </Badge>
                            {currentGuardStatus?.message && (
                                <span className="text-xs text-gray-500">
                                    {currentGuardStatus.message}
                                </span>
                            )}
                        </div>
                        {currentGuardStatus?.guard && (
                            <div className="flex items-center gap-2 text-xs text-gray-500">
                                <User className="h-3 w-3" />
                                <span>ID: {currentGuardStatus.guard.id}</span>
                                <span className="text-gray-300">|</span>
                                <span>{currentGuardStatus.guard.name}</span>
                            </div>
                        )}
                        {isStatusLoading && (
