...
export function DutyCreateForm({
    ...
    const [timeTypeCreateDialogOpen, setTimeTypeCreateDialogOpen] = useState(false)
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            dispatch(fetchClients({ page: 1, per_page: 100, is_active: true }))
        }
    }, [dispatch, isOpen])
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            if (timeTypeSearch.trim() || timeTypeSearch === "") {
                dispatch(fetchDutyTimeTypes({
                    page: 1,
                    per_page: 10,
                    is_active: true,
                    search: timeTypeSearch.trim()
                }))
            }
        }, 300)
    ...
    // Custom render for Duty Time Type dropdown with plus icon
    const renderTimeTypeDropdown = () => (
        <div className="space-y-2">
            ...
            <div className="flex gap-2">
                ...
                <Button
                    ...
                    size="icon"
                    className="shrink-0 h-10 w-10"
                    onClick={() => setTimeTypeCreateDialogOpen(true)}
                    disabled={isLoading}
                    title="Create new duty time type"
                >
                    <Plus className="h-4 w-4" />
                </Button>
            </div>
    ...
    return (
        <>
            ...
            <DutyTimeTypeCreateForm
                ...
                onOpenChange={setTimeTypeCreateDialogOpen}
            />
        </>
    )
}
