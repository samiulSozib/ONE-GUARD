// components/company-service-unit-type/company-service-unit-type-create-form.tsx
"use client";
...
interface CompanyServiceUnitTypeCreateFormProps {
  trigger: ReactNode;
  isOpen?: boolean;
...
export function CompanyServiceUnitTypeCreateForm({
  trigger,
  isOpen,
  onOpenChange,
  onSuccess
}: CompanyServiceUnitTypeCreateFormProps) {
  const dispatch = useAppDispatch();
  const [isLoading, setIsLoading] = useState(false);
  const { error } = useAppSelector((state) => state.companyServiceUnitType);
  ...
  const onSubmit = async (data: UnitTypeFormData) => {
    setIsLoading(true);
    try {
      const submitData: CreateCompanyServiceUnitTypeDto = {
        name: data.name.trim(),
      ...
      const result = await dispatch(createCompanyServiceUnitType(submitData));

      if (createCompanyServiceUnitType.fulfilled.match(result)) {
      ...
      let errorMessage = "Failed to create unit type. Please try again.";
    ...
    <Dialog open={isOpen} onOpenChange={handleDialogOpenChange}>
      ...
      <DialogContent className="sm:max-w-[600px] w-[90vw] max-w-[90vw] mx-auto max-h-[85vh] overflow-y-auto dark:bg-gray-900 p-6">
        ...
        <div className="flex items-center gap-2 text-lg font-semibold mb-6 pb-2 border-b">
          ...
          <span className="whitespace-nowrap">Add New Unit Type</span>
        </div>
