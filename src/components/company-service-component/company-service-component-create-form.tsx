// components/company-service-component/company-service-component-create-form.tsx
"use client";
...
import { createCompanyServiceComponent } from "@/store/slices/company-service-component.slice";
import { fetchCompanyServices } from "@/store/slices/company-service.slice";
import { CreateCompanyServiceComponentDto } from "@/app/types/company-service-component";
...
interface CompanyServiceComponentCreateFormProps {
  trigger: ReactNode;
  isOpen?: boolean;
...
export function CompanyServiceComponentCreateForm({
  trigger,
  isOpen,
  onOpenChange,
  onSuccess,
  defaultParentId
}: CompanyServiceComponentCreateFormProps) {
  ...
  const { companyServiceComponents } = useAppSelector((state) => state.companyServiceComponent);
  ...
  const {
    ...
    defaultValues: {
      parent_company_service_id: defaultParentId || undefined,
  ...
  useEffect(() => {
    if (formValues.default_quantity && formValues.default_quantity > 0) {
      setValue('included_quantity', formValues.default_quantity);
    }
  }, [formValues.default_quantity, setValue]);

  const onSubmit = async (data: ComponentFormData) => {
    setIsLoading(true);
    try {
      const submitData: CreateCompanyServiceComponentDto = {
      ...
      const result = await dispatch(createCompanyServiceComponent(submitData));

      if (createCompanyServiceComponent.fulfilled.match(result)) {
        SweetAlertService.success(
          'Component Added',
          `Component has been added to the package successfully.`
        ).then(() => {
          reset();
          onSuccess?.();
          onOpenChange?.(false);
  ...
  // Get available component services (exclude parent and already added components)
  const getAvailableComponents = () => {
    const parentId = formValues.parent_company_service_id;
    if (!parentId) return companyServices;

    // Get component IDs already added to this parent
    const existingComponentIds = companyServiceComponents
      .filter(c => c.parent_company_service_id === parentId)
      .map(c => c.component_company_service_id);

    // Filter out parent service and already added components
    ...
    <Dialog open={isOpen} onOpenChange={handleDialogOpenChange}>
      ...
      <DialogContent className="sm:max-w-[800px] w-[95vw] max-w-[95vw] mx-auto max-h-[90vh] overflow-y-auto dark:bg-gray-900 p-6">
        ...
        <div className="flex items-center gap-2 text-lg font-semibold mb-6 pb-2 border-b">
          ...
          <span className="whitespace-nowrap">Add Component to Package</span>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <SearchableDropdownWithIcon
              ...
              onValueChange={(value) => {
                ...
                setValue("component_company_service_id", 0);
              }}
          ...
          <div className="space-y-2">
            ...
            <SearchableDropdownWithIcon
              ...
              disabled={isLoading || !formValues.parent_company_service_id}
            ...
            {formValues.parent_company_service_id && (
              <p className="text-xs text-gray-500">
                <AlertCircle className="h-3 w-3 inline mr-1" />
                Only services not already added to this package are shown
              </p>
            )}
          </div>
