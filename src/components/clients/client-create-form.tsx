...
export function ClientCreateForm({
    ...
    const handleMapLocationSelect = useCallback((lat: number, lng: number, address: string) => {
        if (mapPickerTarget.siteIndex !== undefined) {
            if (mapPickerTarget.locationIndex !== undefined) {
                // Update location within site
                handleFieldChange(`sites.${mapPickerTarget.siteIndex}.locations.${mapPickerTarget.locationIndex}.latitude`, lat)
                handleFieldChange(`sites.${mapPickerTarget.siteIndex}.locations.${mapPickerTarget.locationIndex}.longitude`, lng)
            } else {
                // Update site coordinates
                handleFieldChange(`sites.${mapPickerTarget.siteIndex}.latitude`, lat)
                handleFieldChange(`sites.${mapPickerTarget.siteIndex}.longitude`, lng)
                if (address) {
                    handleFieldChange(`sites.${mapPickerTarget.siteIndex}.address`, address)
                }
            }
        }
        setShowMapPicker(false)
        setMapPickerTarget({})
        SweetAlertService.success('Location Selected', 'Location has been updated successfully.', { timer: 1500, showConfirmButton: false })
    ...
    const onSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Sites
            const sitesToSend = formData.sites.map(({ ...site }) => ({
                ...site,
                locations: site.locations.map(({ ...location }) => location)
            }))
            ...
            {showMapPicker && (
                <GoogleMapPicker
                    ...
                    onSelect={(lat: number, lng: number, address: string) => {
                        ...
                        if (siteIndex !== undefined) {
                            if (locationIndex !== undefined) {
                                ...
                                if (address) handleFieldChange(`sites.${siteIndex}.address`, address)
                            }
                        }
                        setShowMapPicker(false)
                        setMapPickerTarget({})
                    ...
                    initialLat={mapPickerTarget.siteIndex !== undefined ?
                        formData.sites[mapPickerTarget.siteIndex]?.latitude || 23.6850 : 23.6850}
                    initialLng={mapPickerTarget.siteIndex !== undefined ?
                        formData.sites[mapPickerTarget.siteIndex]?.longitude || 90.3563 : 90.3563}
                />
            )}
        </>
    )
