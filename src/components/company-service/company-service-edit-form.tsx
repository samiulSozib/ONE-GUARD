// components/company-service/company-service-edit-form.tsx
"use client";
...
import { CompanyService, UpdateCompanyServiceDto } from "@/app/types/company-service";
...
interface CompanyServiceEditFormProps {
  trigger: ReactNode;
...
export function CompanyServiceEditForm({
  trigger,
  service,
  isOpen,
  onOpenChange,
  onSuccess
}: CompanyServiceEditFormProps) {
  ...
  const loadService = async () => {
    ...
    setIsFetching(true);
    try {
      const result = await dispatch(fetchCompanyService(service.id));

      if (fetchCompanyService.fulfilled.match(result)) {
      ...
      SweetAlertService.error('Error', 'Failed to load service details');
    } finally {
      setIsFetching(false);
    }
  };
  ...
  const onSubmit = async (data: ServiceFormData) => {
    ...
    setIsLoading(true);
    try {
      const submitData: UpdateCompanyServiceDto = {
        name: data.name.trim(),
      ...
      const result = await dispatch(updateCompanyService({
        id: service.id,
        data: submitData
      }));

      if (updateCompanyService.fulfilled.match(result)) {
  ...
  const handleDialogOpenChange = (open: boolean) => {
    if (open) {
      ...
      const originalData = {
        name: service?.name || "",
        code: service?.code || "",
        company_service_category_id: service?.company_service_category_id,
        company_service_unit_type_id: service?.company_service_unit_type_id,
        company_service_billing_method_id: service?.company_service_billing_method_id,
        service_type: service?.service_type || "standalone",
        description: service?.description || "",
        default_selling_rate: service?.default_selling_rate?.toString() || "",
        default_internal_cost: service?.default_internal_cost?.toString() || "",
      ...
      const currentData = {
        ...
        company_service_category_id: formValues.company_service_category_id,
        company_service_unit_type_id: formValues.company_service_unit_type_id,
      ...
      const hasChanges =
        ...
        currentData.company_service_category_id !== originalData.company_service_category_id ||
        currentData.company_service_unit_type_id !== originalData.company_service_unit_type_id ||
        currentData.company_service_billing_method_id !== originalData.company_service_billing_method_id ||
        currentData.service_type !== originalData.service_type ||
    ...
    <Dialog open={isOpen} onOpenChange={handleDialogOpenChange}>
      ...
      <DialogContent className="sm:max-w-[900px] w-[95vw] max-w-[95vw] mx-auto max-h-[90vh] overflow-y-auto dark:bg-gray-900 p-6">
        ...
        <div className="flex items-center gap-2 text-lg font-semibold mb-6 pb-2 border-b">
          ...
          <span className="whitespace-nowrap">Edit Service</span>
        </div>
